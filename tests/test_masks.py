...
def test_invalid_mask_card_number(invalid_card_number):
    for number in invalid_card_number:
        assert get_mask_card_number(number) == "Не верный номер карты"


def test_valid_mask_card_number(valid_card_number):
...
def valid_account_number():
    return [
        ...
        ("65342678901234563445", "**3445"),
        ("45675688901234563987", "**3987")
    ]


def test_invalid_mask_card_account(invalid_account_number):
    for number in invalid_account_number:
        assert get_mask_account(number) == "Не верный номер счета"

def test_valid_account_number(valid_account_number):
